...
import '../activity/widgets/activities_section_widget.dart';
...
class _DailyTrackingDashboardState extends State<DailyTrackingDashboard> {
  ...
  int _slideDirection = 0; // -1: dia anterior, +1: próximo dia
  double _dragX = 0.0; // deslocamento atual do arrasto horizontal
  ...
  // Activities (Health): passos e energia ativa
  int _stepsToday = 0;
  double _activeKcalToday = 0.0;
  ...
  void didChangeDependencies() {
    ...
    _refreshGamificationRow();
    // Opcional: não chamamos Health aqui para evitar prompt automático.
  }
  ...
  Future<void> _refreshActivities() async {
    try {
      ...
      final s = await svc.stepsForDay(_selectedDate);
      final k = await svc.activeEnergyForDay(_selectedDate);
      if (!mounted) return;
      setState(() {
        _stepsToday = s;
        _activeKcalToday = k;
  ...
  Widget _fastingStatusCard(BuildContext context) {
    final colors = context.colors;
    final text = context.textStyles;

    final DateTime? endAt = _fastEndAt;
    String title;
    String subtitle;
    IconData icon;
    ...
    if (endAt != null) {
      ...
      final bool sameDay = endAt.year == now.year &&
          endAt.month == now.month &&
          endAt.day == now.day;
      final String timeLabel = '${two(endAt.hour)}:${two(endAt.minute)}';
      final String datePrefix =
          sameDay ? 'hoje' : '${two(endAt.day)}/${two(endAt.month)}';

      title = 'Jejum em andamento';
      subtitle = 'Termina $datePrefix às $timeLabel.';
      icon = Icons.timer_outlined;
    } else {
      title = 'Nenhum jejum ativo';
      subtitle = 'Inicie um jejum na aba Jejum.';
  ...
  Widget _waterStreakChip() {
    ...
    final label =
        _hydrationStreak > 0 ? '${_hydrationStreak}d de água' : 'Sem sequência de água';
  ...
  Widget _fastingStreakChip() {
    ...
    final label =
        _fastingStreak > 0 ? '${_fastingStreak}d de jejum' : 'Sem sequência de jejum';
    return _buildStreakChip(
      icon: Icons.local_fire_department,
      baseColor: context.semanticColors.warning,
      label: label,
                ...
                Expanded(
                  child: Builder(
                    builder: (context) {
                      ...
                      return Text(
                        until != null
                            ? 'Jejum em andamento, notificações pausadas até $untilLabel.$endLabel$schLabel'
                            : 'Jejum em andamento, notificações pausadas.$endLabel$schLabel',
  ...
  Widget _calorieBudgetCard(
    ...
    if (goal <= 0 || (food == 0 && exercise == 0)) {
      summaryText =
          'Comece registrando suas refeições para ver o resumo aqui.';
      summaryColor = cs.onSurfaceVariant;
    } else if (exceeded) {
      summaryText =
          'Você passou da meta em ${_fmtInt(absRemaining)} kcal hoje.';
      summaryColor = cs.error;
    } else if (absRemaining == 0) {
      summaryText = 'Meta de hoje batida. Muito bom!';
      summaryColor = cs.primary;
    } else if (absRemaining <= 150) {
      summaryText =
          'Faltam apenas ${_fmtInt(absRemaining)} kcal para a meta de hoje.';
      summaryColor = cs.primary;
    } else {
      summaryText =
          'Você ainda tem ${_fmtInt(absRemaining)} kcal disponíveis hoje.';
      summaryColor = cs.onSurface;
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        // Fundo azul claríssimo uniforme (estilo YAZIO - sem gradient)
        color: cs.primary.withValues(alpha: 0.05),
        borderRadius: BorderRadius.circular(16),
        // Removido: border e boxShadow para design flat como YAZIO
      ),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          ...
          Row(
            ...
            children: [
              ...
              CalorieRing(
                ...
                size: 165,  // Grande e visível
                thickness: 15,  // Ajustado: não muito grosso, não muito fino (V3)
                showTicks: false,
                gapDegrees: 40,
              ),
          ...
          // Resumo textual amigável do dia
          Align(
  ...
  // Helper para macros inline (dentro do card de calorias) - V3
  ...
  Widget _buildMealStatusRow(BuildContext context) {
    ...
    if (hour >= 6 && hour < 10) {
      status = 'Café da manhã';  // Traduzido para português
      icon = Icons.free_breakfast;
    } else if (hour >= 12 && hour < 14) {
      status = 'Almoço';  // Traduzido para português
      icon = Icons.lunch_dining;
    } else if (hour >= 19 && hour < 21) {
      status = 'Jantar';  // Traduzido para português
      icon = Icons.dinner_dining;
    } else {
      status = 'Comendo';  // Traduzido para português
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: cs.primary.withValues(alpha: 0.08),  // Mudado de verde (warningAmber) para azul (primary)
      ...
      child: Row(
        ...
        children: [
          Icon(
            icon,
            size: 16,
            color: cs.primary,  // Mudado de warningAmber para primary (azul)
          ),
          const SizedBox(width: 6),
          Text(
            'Agora: $status',  // Traduzido "Now" para "Agora"
            style: TextStyle(
              fontSize: 13.sp,
              fontWeight: FontWeight.w600,
              color: cs.primary,  // Mudado para primary (azul)
            ),
          ),
        ],
      ),
        ...
        child: RefreshIndicator(
          ...
          child: GestureDetector(
            ...
            onHorizontalDragEnd: (_) {
              ...
              _dragX = 0.0;
              if (dx.abs() > threshold) {
                // Invertido: arrastar para a esquerda avança data (+1),
                // arrastar para a direita reduz data (-1)
                final dir = dx > 0 ? -1 : 1;
                _shiftDay(dir);
              } else {
                setState(() {}); // apenas repinta para voltar ao zero
              }
            },
            child: AnimatedSwitcher(
              ...
              transitionBuilder: (child, animation) {
                final begin = (_slideDirection == 0)
                    ...
                    : Offset(_slideDirection > 0 ? 1.0 : -1.0, 0.0);
              ...
              child: Transform.translate(
                ...
                Padding(
                  ...
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // Coluna esquerda: Data atual + Week X
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // Data selecionada: "Today" se for hoje, senão mostra a data
                          Text(
                            () {
                              final now = DateTime.now();
                              ...
                              if (selected == today) {
                                ...
                                // Formato: "15 Jan" ou "15 Jan 2024" se ano diferente
                                final months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                                final dateStr = '${_selectedDate.day} ${months[_selectedDate.month - 1]}';
                                return _selectedDate.year != now.year ? '$dateStr ${_selectedDate.year}' : dateStr;
                              }
                            }(),
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Section header like YAZIO: Resumo do dia // |  Ver detalhes
                      ...
                      // Macros agora estão DENTRO do card de calorias (V3)
                    ],
                  ),
                ),
                ...
                Padding(
                  padding: EdgeInsets.symmetric(horizontal: 4.w),
                  child: const DashboardSectionHeader(
                    title: 'Água',
                  ),
                ),
                ...
                Padding(
                  ...
                  child: DashboardSectionHeader(
                    ...
                    actionLabel: 'Ver todas as medidas',
                    onAction: () {
                      Navigator.pushNamed(context, AppRoutes.bodyMetrics);
                    },
                  ),
                  ...
                  child: FutureBuilder<List<Object?>>(
                    ...
                    builder: (context, snap) {
                      ...
                      return BodyMetricsCard(
                        ...
                        bpSys: (m['bloodPressureSys'] as num?)?.toInt(),
                        bpDia: (m['bloodPressureDia'] as num?)?.toInt(),
                        glucoseMgDl: (m['bloodGlucoseMgDl'] as num?)?.toInt(),
                        bpSysSeries7: [
                          for (final e in recent)
                            if ((e.$2['bloodPressureSys'] as num?) != null)
                              (e.$2['bloodPressureSys'] as num).toDouble(),
                        ],
                        glucoseSeries7: [
                          for (final e in recent)
                            if ((e.$2['bloodGlucoseMgDl'] as num?) != null)
                              (e.$2['bloodGlucoseMgDl'] as num).toDouble(),
                        ],
                        onEditBloodPressure: () async {
                          final sysCtl = TextEditingController(text: (m['bloodPressureSys'] as num?)?.toInt().toString() ?? '');
                          final diaCtl = TextEditingController(text: (m['bloodPressureDia'] as num?)?.toInt().toString() ?? '');
                            ...
                            builder: (ctx) {
                              return Padding(
                                ...
                                child: Column(mainAxisSize: MainAxisSize.min, children: [
                                  ...
                                  const SizedBox(height: 12),
                                  Row(children: [
                                    Expanded(child: TextField(controller: sysCtl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Sistólica (mmHg)', isDense: true))),
                                    const SizedBox(width: 12),
                                    Expanded(child: TextField(controller: diaCtl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Diastólica (mmHg)', isDense: true))),
                                  ]),
                                  const SizedBox(height: 12),
                                  SizedBox(width: double.infinity, child: ElevatedButton(
                                    onPressed: () async {
                                      int? sys = int.tryParse(sysCtl.text.trim());
                                      int? dia = int.tryParse(diaCtl.text.trim());
                                      if (sys == null || dia == null) {
                                        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Informe uma pressão válida.')));
                                        return;
                                      }
                                      if (sys < 60 || sys > 220 || dia < 30 || dia > 140 || sys <= dia) {
                                        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Faixa de pressão inválida. Exemplo: 120 x 80.')));
                                        return;
                                      }
                                      final cur = await BodyMetricsStorage.getForDate(_selectedDate);
                                      cur['bloodPressureSys'] = sys;
                                      cur['bloodPressureDia'] = dia;
                                      await BodyMetricsStorage.setForDate(_selectedDate, cur);
                                      if (!mounted) return;
                                      Navigator.pop(ctx);
                                      setState(() {});
                                    },
                        ...
                        onEditGlucose: () async {
                          final glCtl = TextEditingController(text: (m['bloodGlucoseMgDl'] as num?)?.toInt().toString() ?? '');
                          await showModalBottomSheet(
                            context: context,
                            isScrollControlled: true,
                            shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(16))),
                            builder: (ctx) {
                              return Padding(
                                ...
                                child: Column(mainAxisSize: MainAxisSize.min, children: [
                                  Container(width: 40, height: 4, decoration: BoxDecoration(color: Theme.of(ctx).colorScheme.outlineVariant, borderRadius: BorderRadius.circular(2))),
                                  const SizedBox(height: 12),
                                  TextField(controller: glCtl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Glicose (mg/dL)', isDense: true)),
                                  const SizedBox(height: 12),
                                  SizedBox(width: double.infinity, child: ElevatedButton(
                                    onPressed: () async {
                                      int? g = int.tryParse(glCtl.text.trim());
                                      if (g == null || g < 40 || g > 400) {
                                        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Informe um valor de glicose válido (40-400 mg/dL).')));
                                        return;
                                      }
                                      final cur = await BodyMetricsStorage.getForDate(_selectedDate);
                                      cur['bloodGlucoseMgDl'] = g;
                                      await BodyMetricsStorage.setForDate(_selectedDate, cur);
                                      if (!mounted) return;
                                      Navigator.pop(ctx);
                                      setState(() {});
                ...
                AnimatedCard(
                  ...
                  child: ActivitiesSectionWidget(
                    steps: _stepsToday > 0 ? _stepsToday : null,
                    kcal: _activeKcalToday > 0 ? _activeKcalToday : null,
                    onConnect: () async {
                      final svc = HealthService();
                      ...
                      if (!ok) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Não foi possível conectar. Confira as permissões ou o Health Connect/HealthKit.')),
                        );
                        return;
                      }
                      ...
                      if (!mounted) return;
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Conectado · ${_stepsToday} passos · ${_activeKcalToday.toStringAsFixed(0)} kcal')),
                      );
                    },
                    onManual: () {
...
// Notes card - ÚLTIMO CARD (estilo YAZIO)
  ...
  Future<void> _shiftDay(int deltaDays) async {
    setState(() {
      _slideDirection = deltaDays.sign;
      final d = _selectedDate.add(Duration(days: deltaDays));
      _selectedDate = DateTime(d.year, d.month, d.day);
    });
    await _loadToday();
    await _loadWeek();
