...
class _NotesScreenState extends State<NotesScreen> {
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() => _all = list..sort((a, b) => ((b['updatedAt'] ?? '') as String).compareTo((a['updatedAt'] ?? '') as String)));
    if (_openEditorOnStart) {
      _openEditorOnStart = false;
      if (mounted) WidgetsBinding.instance.addPostFrameCallback((_) => _openEditor());
    }
  }
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    if (t.isNotEmpty) {
      notes = notes.where((n) => ((n['title'] ?? '') as String).toLowerCase().contains(t) || ((n['content'] ?? '') as String).toLowerCase().contains(t));
    }
  ...
  Map<String, List<Map<String, dynamic>>> get _groupedByDay {
    ...
    for (final n in _filtered) {
      final d = DateTime.tryParse((n['updatedAt'] ?? n['createdAt']) as String? ?? '') ?? DateTime.now();
      final key = '${d.year.toString().padLeft(4, '0')}-${d.month.toString().padLeft(2, '0')}-${d.day.toString().padLeft(2, '0')}';
      (out[key] ??= []).add(n);
    }
    // sort keys desc
    final sorted = Map.fromEntries(out.entries.toList()
  ...
  Widget _dayHeader(String ymd) {
    ...
    if (parts.length == 3) {
      ...
      if (dd == today) {
        ...
        label = '${dd.day.toString().padLeft(2, '0')}/${dd.month.toString().padLeft(2, '0')}';
      }
    } else {
      label = ymd;
    }
  ...
  Widget _noteTile(Map<String, dynamic> n) {
    ...
    final updated = DateTime.tryParse((n['updatedAt'] ?? n['createdAt']) as String? ?? '') ?? DateTime.now();
    final dateStr = '${updated.hour.toString().padLeft(2, '0')}:${updated.minute.toString().padLeft(2, '0')}';
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text((n['title'] as String?)?.trim().isNotEmpty == true ? (n['title'] as String) : 'Sem título',
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return ClipRRect(
                                ...
                                child: (p is String && File(p).existsSync())
                                    ? Image.file(File(p), width: 56, height: 56, fit: BoxFit.cover)
                                    : Container(width: 56, height: 56, color: cs.outlineVariant.withValues(alpha: 0.2), child: const Icon(Icons.image_not_supported)),
                              );
                            },
                          ),
                        ),
                        ...
                        children: [
                          ...
                          Flexible(
                            child: Wrap(
                              ...
                              runSpacing: 6,
                              children: [
                                for (final t in ((n['tags'] as List?) ?? const []))
                                  Chip(label: Text('$t'), visualDensity: VisualDensity.compact),
                              ],
                            ),
                          ),
                          Text(dateStr, style: Theme.of(context).textTheme.labelSmall?.copyWith(color: cs.onSurfaceVariant)),
                        ],
                      ),
                    ],
                  ),
          ...
          children: [
            ...
            Expanded(
              child: _filtered.isEmpty
                  ? Center(
                      child: Text('Sem anotações', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: cs.onSurfaceVariant)),
                    )
                  : ListView(
                      children: [
          ...
          children: [
            ...
            Align(
              ...
              child: Wrap(
                ...
                runSpacing: 6,
                children: [
                  for (final t in ['Geral','Café da manhã','Almoço','Jantar','Lanches','Exercício','Água'])
                    FilterChip(
                      label: Text(t),
                      selected: tags.contains(t),
                      onSelected: (v) => setState(() { v ? tags.add(t) : tags.remove(t); }),
                    ),
                ],
              ),
            ),
            ...
            Row(
              children: [
                ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: AppTheme.activeBlue, foregroundColor: Colors.white),
                ),
                const SizedBox(width: 8),
                OutlinedButton.icon(
                ...
                const SizedBox(width: 8),
                Text('${attachments.length} anexos', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: cs.onSurfaceVariant)),
              ],
            ),
            const SizedBox(height: 6),
                    ...
                    children: [
                      ClipRRect(
                        ...
                        child: File(attachments[i]).existsSync()
                            ? Image.file(File(attachments[i]), width: 70, height: 70, fit: BoxFit.cover)
                            : Container(width: 70, height: 70, color: cs.outlineVariant.withValues(alpha: 0.2), child: const Icon(Icons.image_not_supported)),
                      ),
                      Positioned(
                        right: 2,
                        top: 2,
                        child: InkWell(
                          onTap: () => setState(() => attachments.removeAt(i)),
...
extension on _NoteEditorState {
  Future<void> _pickImages() async {
    try {
      ...
      for (final f in files) {
        final String name = 'img_${DateTime.now().millisecondsSinceEpoch}_${attachments.length}.jpg';
  ...
  Future<void> _captureImage() async {
    try {
      final ImagePicker picker = ImagePicker();
      final XFile? file = await picker.pickImage(source: ImageSource.camera, imageQuality: 90);
      if (file == null) return;
      final dir = await getApplicationDocumentsDirectory();
      final notesDir = Directory('${dir.path}/notes');
      if (!notesDir.existsSync()) notesDir.createSync(recursive: true);
      final String name = 'cam_${DateTime.now().millisecondsSinceEpoch}_${attachments.length}.jpg';
      final String dest = '${notesDir.path}/$name';
